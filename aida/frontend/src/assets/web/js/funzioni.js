...
// MARIO MOLINARA
function isConstantAsInstruction(instruction){
    return instruction[1] == 'a' && instruction[2] == 'fromM' || instruction[1] == 'A/' && instruction[2] == 'fromM'
    //return instruction && instruction[1] == 'a'  && instruction[2] == 'fromM' && instruction[0] && instruction[0].length > 0;
}
function isJumpInstruction(instruction){
    return instruction[2] == 'V' || instruction[2] == 'W' || instruction[2] == 'Y' || instruction[2] == 'Z';
}
// MARIO MOLINARA


//Controllo se registro è disponibile:
function isAvailable(letter) {
    if (letter == null) {
        return true
    }
...
function storeInRegister(value, letter) {
    ...
    var MAX_VALUE_22 = 9999999999999999999999;
    var MAX_VALUE_11 = 99999999999;

    if (value > MAX_VALUE_22 || value < -MAX_VALUE_22 || value.toString().length > 22) {
    ...
    if (value > MAX_VALUE_11 || value < -MAX_VALUE_11 || valueLenght > 11) {
    ...
    if (destination == 'R'){
        value = toFixedWithTruncation_R(value);
    }
...
function truncateDecimals(number, digits) {
    ...
    if (number){
        if (number instanceof Big){
            output = truncateDecimals_big(number, digits);
        } else {
            var toInt = parseFloat(number);
            console.log(number);
            if (!isNaN(toInt)){
                output = toInt.toFixed(digits);
            }
        }
    }
    return output;
...
/*
function toFixedWithTruncation_old(number){
    var toFixed = 0;
    var i = 20;
    while(i > 0 && (parseFloat(number.toFixed(i)) <= parseFloat(number.toFixed(i-1)))){
        i--;
    }
...
function toFixedWithTruncation_R(number){
    if ((typeof number) == "string" && number.indexOf(".") == -1){
        return number;
    }

    return toFixedWithTruncation(number, 20);
}

function toFixedWithTruncation(number, maxLen){
    var fixed = number.toFixed(maxLen);
    ...
    if (finalIndex > 0){
        fixed = fixed.substring(0, finalIndex);
    } else {
        fixed = "0";
    }
...
function truncateDecimals_big(number, digits) {
    if (!number) {
        number = 0;
    }
