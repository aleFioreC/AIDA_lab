...
function execute(instruction){
    ...
    /*
    if(!isConstantAsInstruction(instruction) && !isAvailable(instruction[1])){
        return;
    }*/

    // MARIO MOLINARA
    // The availability is verified when:
    //  - is a jump instruction
    //  - is a constant as instruction
    if(!isConstantAsInstruction(instruction) && !isJumpInstruction(instruction) && !isAvailable(instruction[1])){
        return;
    }
    // MARIO MOLINARA
	...
	if (instruction[2] == 'div') {
        ...
        storeInRegister(R, 'R');
        storeInRegister(parseFloat(truncateDecimals(quoziente, statoMacchina.numberOfDecimals)), 'A')
    }
